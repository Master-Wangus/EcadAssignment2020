...
if($_POST) //Post Data received from Shopping cart page.
{
	...
	$conn->connect();
		foreach($_SESSION['Items'] as $key=>$item) 
		{
			...
			if ($item["quantity"] > $row["Quantity"])
			{
				...
				$MainContent .= "Please resturn to shopping cart to amend your purchase.<br/>";
	...
	$padata = '&CURRENCYCODE='.urlencode($PayPalCurrencyCode).
			  ...
			  '&BRANDNAME='.urlencode("Johnson Johnson").
			  $paypal_data.				
...
if(isset($_GET["token"]) && isset($_GET["PayerID"])) {	
	...
	if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || 
	   "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {
		...
		$qry = "SELECT Quantity, ProductID FROM shopcartitem WHERE ShopCartID = $_SESSION[Cart]";
		$result = $conn->query($qry);
		if ($result->num_rows > 0) 
		{
			while ($row = $conn->fetch_array($result))
			{
				$noquantity = $row[Quantity];
				$productID = $row[ProductID];
				$qry = "UPDATE product SET Quantity = Quantity - $noquantity WHERE ProductID = $productID";
				$conn->query($qry);
			}
		}
		...
		if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || 
		   "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {
			...
			VALUES('$ShipName', '$ShipAddress','$ShipCountry','$ShipEmail',$_SESSION[Cart])";
			$conn->query($qry);
